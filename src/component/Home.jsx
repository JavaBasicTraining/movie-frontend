...
const filterMovie = async (params)=>
  {
    try {
     const response  =  await axiosInstance.get(`/api/v1/movies/filter/${params}`);
      ...
      <div className='header'>
        <div className='header-title'>
          ...
          <div className='search'>
            <input placeholder='Tim Kiếm Phim' className='search-input'
               type="text"
               name="name"
               value={name}
               ...
               required
           ></input>
            <button onClick={()=> filterMovie(name)}> <SearchOutlined /></button>
          </div>
        </div>
